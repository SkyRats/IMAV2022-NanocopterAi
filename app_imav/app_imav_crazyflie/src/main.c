        ...
        if (dma_flag == 1){
            ...
            case FORWARD: //Goes forward until it reaches arena edges.
                if(xEstimate <= ARENA_EDGES && xEstimate >= -ARENA_EDGES && yEstimate <= ARENA_EDGES && yEstimate >= -ARENA_EDGES){
                    if (squareX != 0 && squareY != 0 && velocityPred > 0.3f){//velocity predicitoon depends on max vel
                        if (squareX >= 110){
                            vTaskDelay(M2T(10));
                            setVelocitySetPoint(&setpoint, FORWARD_VEL, -SIDEWAYS_VEL, 0, 0);
                            commanderSetSetpoint(&setpoint, 3);
                            vTaskDelay(M2T(200));
                        }
                        else if(squareX <= 90){
...
void appMain(){
    ...
    while(positioningInit && turnOff == false && aiInit){
        if (dma_flag == 1){
            ...
            if (velocityPred <= 0.4f && takeoffCounter >= 10){
            ...
            case FORWARD: //Goes forward until it reaches arena edges.
                if(xEstimate <= ARENA_EDGES && xEstimate >= -ARENA_EDGES && yEstimate <= ARENA_EDGES && yEstimate >= -ARENA_EDGES){
                    if (squareX != 0 && squareY != 0){//velocity predicitoon depends on max vel
                        if (squareX >= 110){
                            vTaskDelay(M2T(10));
                            setVelocitySetPoint(&setpoint, FORWARD_VEL, -SIDEWAYS_VEL, 0, 0);
                            commanderSetSetpoint(&setpoint, 3);
            ...
            case OBSTACLE_ROTATE:
                ...
                if (ABS(delta) <= 10.0f && velocityPred > 0.4f){
                    commanderRelaxPriority();
                    vTaskDelay(M2T(10));
                    setVelocitySetPoint(&setpoint, 0, 0, 0, 0);
                    vTaskDelay(M2T(1000));
                ...
                else if((squareX != 0 && squareY != 0) && velocityPred > 0.4f){
                    commanderRelaxPriority();
                    vTaskDelay(M2T(10));
                    setVelocitySetPoint(&setpoint, FORWARD_VEL, 0, 0, 0);
                    vTaskDelay(M2T(500));
